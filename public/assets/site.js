(() => {
 ...
 document.querySelectorAll('[data-dialog]').forEach(button=>button.addEventListener('click',()=>{const [title,text]=messages[button.dataset.dialog];document.getElementById('dialog-title').textContent=title;const p=document.createElement('p');p.textContent=text;document.getElementById('dialog-body').replaceChildren(p);dialog.showModal();}));
